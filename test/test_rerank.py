...
def _render(T):
    ...
    depth = np.where(np.isfinite(zb) & sil, np.where(np.isfinite(zb), zb, 0), 0)
    return sil, depth


MASK, DEPTH_TRUE = _render(T_TRUE)
# a dense depth image: the vertex z-buffer is sparse, so fill the object's
# silhouette with its nearest valid depth (nearest-neighbour), table at 0.9 m
from scipy.ndimage import distance_transform_edt                # noqa: E402
_valid = DEPTH_TRUE > 0
_idx = distance_transform_edt(~_valid, return_distances=False, return_indices=True)
DEPTH = np.where(MASK, DEPTH_TRUE[_idx[0], _idx[1]], 0.9)


def _hypothesis_set(include_correct=True, tumbled=True, filler=True, n_wrong=10, rng_seed=0):
    """scorer-sorted: n_wrong wrong-yaw jittered (handle behind), then (optionally)
    5 correct-yaw, then (optionally) 3 tumbled, then (optionally) filler at other yaws."""
    ...
    sils = np.stack([_render(T)[0] for T in poses])
    return sils, scores, poses
...
def test_selects_correct_yaw_cluster():
    sils, scores, poses = _hypothesis_set()
    chosen, rec = rerank_hypotheses(sils, scores, poses, MASK, DEPTH, DIAMETER)
...
def test_deterministic():
    sils, scores, poses = _hypothesis_set()
    a = rerank_hypotheses(sils, scores, poses, MASK, DEPTH, DIAMETER)[0]
    b = rerank_hypotheses(sils, scores, poses, MASK, DEPTH, DIAMETER)[0]
    assert a == b
...
def test_declines_when_nothing_unexplained():
    ...
    chosen, rec = rerank_hypotheses(sils, scores, poses, mask_no_handle, None, DIAMETER)
...
def test_declines_when_no_hypothesis_reaches_U():
    ...
    sils, scores, poses = _hypothesis_set(include_correct=False, tumbled=False, filler=False, n_wrong=14)
    ...
    chosen, rec = rerank_hypotheses(sils, scores, poses, blob, depth, DIAMETER)
    assert chosen == 0 and not rec.changed
...
def test_tumbled_body_covering_U_is_rejected():
    ...
    sils, scores, poses = _hypothesis_set(tumbled=True)
    chosen, rec = rerank_hypotheses(sils, scores, poses, MASK, DEPTH, DIAMETER)
...
def test_depth_guard_drops_table_leak():
    ...
    chosen_l, rec_l = rerank_hypotheses(sils, scores, poses, leak, DEPTH, DIAMETER)
    chosen, rec = rerank_hypotheses(sils, scores, poses, MASK, DEPTH, DIAMETER)
    ...
    _, rec_ng = rerank_hypotheses(sils, scores, poses, leak, None, DIAMETER)
...
def test_dilation_scales_with_mask_and_band_with_mesh():
    sils, scores, poses = _hypothesis_set()
    _, rec = rerank_hypotheses(sils, scores, poses, MASK, DEPTH, DIAMETER)
